...
/* Forward declarations for SoftRF piezo beeper driver */
extern void PiezoBeeper_setFreq(uint16_t freq_hz);

/* Wrapper for toneAC() interface → PiezoBeeper_setFreq() */
static inline void toneAC(double freq, uint8_t volume = 10) {
  (void)volume;  /* volume is ignored; use Vario settings instead */
  PiezoBeeper_setFreq((uint16_t)freq);
}
